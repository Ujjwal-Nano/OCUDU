...
// compared.  The test always prints a per-step table and a max-error summary
// so the accuracy trade-off between the two propagators is immediately visible
...
TEST(test_converters, compare_rk4_vs_keplerian_leo_5min)
{
  ...
  // Sanity bounds: for 300 s of LEO propagation the J2-driven divergence
  // between Keplerian and RK4 should not exceed ~600 m.
  EXPECT_LT(max_pos_error_m, 700.0);
  EXPECT_LT(max_range_error_m, 700.0);
  EXPECT_LT(max_delay_error_ns, 2400.0);
}

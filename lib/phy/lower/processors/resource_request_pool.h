...
/// - On a collision it returns the request that was passed in, so a producer can report its own dropped slot. The
///   consumer passes an empty request, hence a dropped pop simply yields "no request for this slot".
///

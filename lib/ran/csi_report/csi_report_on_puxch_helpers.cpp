                                                        ...
                                                        bounded_integer<uint8_t, 1, 4> nof_reported_rs_)
{
  ...
  for (unsigned i = 0; i != nof_reported_rs; ++i) {
    result.cri.emplace_back(packed.extract(offset, sizes.cri));
    offset += sizes.cri;
  }

  // Unpack RSRP. Conversion from the 7-bit field to dBm is described in TS38.214 Section 5.2.1.4.3.
  int reference_rsrp = -140 + packed.extract(offset, sizes.rsrp);
  result.rsrp_dBm.emplace_back(reference_rsrp);
  offset += sizes.rsrp;
  ...
  for (unsigned i = 1; i != nof_reported_rs; ++i) {
    result.rsrp_dBm.emplace_back(reference_rsrp - 2 * packed.extract(offset, sizes.diff_rsrp));

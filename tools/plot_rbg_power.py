...
def main():
    ...
    Plin = to_ru_lin(P, a.K)  # (time, R) linear per-RBG power
    M = 10 * np.log10(Plin)  # dB, for panel 1 + best-fraction
    R = M.shape[1]
    label = a.label if a.label else a.cap.split("/")[-1]

    best = M.argmax(1)
    best_frac = np.array([100 * (best == r).mean() for r in range(R)])
    gain = 10 * np.log10(Plin.max(1) / Plin.mean(1))  # best / arithmetic-mean, dB

    fig, ax = plt.subplots(1, 3, figsize=(16, 5))

    # Panel 1: per-RBG power distribution (box plot)
    ax[0].boxplot(
        [M[:, r] for r in range(R)],
        labels=[f"RBG{r}" for r in range(R)],
        showfliers=False,
        patch_artist=True,
        boxprops=dict(facecolor="#8fb3e0"),
    ...
    bars = ax[1].bar(
        [f"RBG{r}" for r in range(R)], best_frac, color="#d62728", alpha=0.8
    )
    for b, f in zip(bars, best_frac):
        ax[1].text(
            b.get_x() + b.get_width() / 2, f + 0.5, f"{f:.0f}%", ha="center", fontsize=9
        )
    ...
    ax[1].set_ylim(0, max(best_frac) * 1.2)
    ...
    ax[2].set_xlabel("swapping gain (dB)")
    ...
    L = [
        f"dataset: {label}   K={a.K} -> {R} RBGs   samples={len(M)}",
        "best-RBG fraction: "
        + ", ".join(f"RBG{r}={best_frac[r]:.1f}%" for r in range(R)),
        ...
        "per-RBG median power (dB): "
        + ", ".join(f"RBG{r}={np.median(M[:,r]):.1f}" for r in range(R)),
    ]
    open(txt, "w").write("\n".join(L) + "\n")
    print("\n".join(L))

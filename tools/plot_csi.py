...
def main():
    ...
    for row, (ukey, rec) in enumerate(sorted(users.items())):
        ...
        if have_t:
            t0 = rec["t"][0]
            tmin = (np.array(rec["t"], dtype=float) - t0) / 60000.0
            xlabel = "time (min since start)"
        else:
            tmin = np.arange(M.shape[0]) / 6000.0
            xlabel = "sample index"
        ...
        if args.zoom_start is None and have_t:
            ...
            if t_hi <= t_lo:
                t_lo, t_hi = tmin[0], tmin[-1] - args.zoom_dur / 60.0
            dur_min = args.zoom_dur / 60.0
            best_start, best_sw = t_lo, -1

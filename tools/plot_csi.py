...
def to_db(a): return 10.0*np.log10(np.maximum(np.asarray(a,dtype=float),EPS))

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("jsonl"); ap.add_argument("-o","--out",default="csi_plots.png")
    args = ap.parse_args()
    ...
    for row,(ukey,rec) in enumerate(sorted(users.items())):
        ...
        if have_t:
            ...
            x = np.arange(M.shape[0]); xlabel = "sample index (no t field)"

        cuts = [0]+[i for i in range(1,len(rn)) if rn[i]!=rn[i-1]]+[len(rn)]
        ax = axes[row][0]
        for r in range(R):
            ax.plot(x, M[:,r], lw=1.0, label=f"RU{r} (RB {12*r}-{12*r+11})")
        for c in cuts[1:-1]:                       # mark re-attach events (rnti change)
            xc = x[c]
            ax.axvline(xc, color="k", ls="--", lw=1, alpha=0.6)
            tlab = f"{xc:.1f}min" if have_t else f"#{c}"
            ax.text(xc, ax.get_ylim()[1], f" re-attach {tlab}\n {hex(int(rn[c]))}",
                    fontsize=6, va="top", ha="left", color="k")
        ...
        im = ax.imshow(M.T, aspect="auto", origin="lower", cmap="viridis",
                       extent=[x[0], x[-1], -0.5, R-0.5])
        ax.set_title("channel matrix [RU x time] (dB)")
        ax.set_xlabel(xlabel); ax.set_ylabel("RU (freq ->)"); ax.set_yticks(range(R))
        fig.colorbar(im, ax=ax, shrink=0.85, label="dB")

        ax = axes[row][2]
        for r in range(R):
            v = np.sort(M[:,r]); c = 1.0-np.arange(1,len(v)+1)/len(v)
            ax.semilogy(v, np.maximum(c,1.0/len(v)), lw=1.2, label=f"RU{r}")
        ax.set_title("CCDF of per-RU CSI"); ax.set_xlabel("CSI power (dB)")
        ax.set_ylabel("P(CSI > x)"); ax.grid(alpha=0.3, which="both"); ax.legend(fontsize=8)

        stats_lines.append(f"\nuser {ukey}: {len(rn)} samples, {len(cuts)-1} segment(s)")
        hdr = "  seg rnti    n      " + "  ".join(f"RU{r}:mean/10%out" for r in range(R))
        stats_lines.append(hdr)
        for si in range(len(cuts)-1):
            a,b = cuts[si], cuts[si+1]; seg = M[a:b]
            cols=[]
            for r in range(R):
                v = np.sort(seg[:,r])
                out10 = v[max(0,int(0.9*len(v))-1)]
                cols.append(f"{seg[:,r].mean():6.1f}/{out10:6.1f}")
            stats_lines.append(f"  {si:>3} {hex(rn[a]):>6} {b-a:>5}  " + "  ".join(cols))

    fig.tight_layout(rect=[0,0,1,0.97]); fig.savefig(args.out, dpi=140)

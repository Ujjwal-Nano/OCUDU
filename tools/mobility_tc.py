...
def tc_at_window(P, fs, win_ms, K=12):
    ...
    m = m - m.mean()
    n=len(m); ac=np.correlate(m,m,mode="full")[n-1:]; ac=ac/ac[0]
...
def find_plateau(P, fs, windows, tol=0.011):
    ...
    n=len(windows)
    for i in range(n):
        if np.isnan(tcs[i]): continue
        j=i
        while j+1<n and not np.isnan(tcs[j+1]) and (max(tcs[i:j+2])-min(tcs[i:j+2]))<=tol:
            j+=1
        run_len=j-i+1
        key=(run_len, -i)
    ...
    if best is not None:
        _,_,i,j=best
        vals=tcs[i:j+1]
        tc=float(np.median(vals)); win=windows[i]
...
def main():
    ...
    print("window sweep:")
    for w,t in sweep:
        mark = " <-- plateau" if (status=='plateau' and not np.isnan(t) and abs(t-tc)<=0.011) else ""
        print(f"  {w:5.0f} ms : Tc = {t:.3f} s{mark}" if not np.isnan(t) else f"  {w:5.0f} ms : Tc = --")
    if status=="plateau":
        ratio = tc/theory if theory==theory else float('nan')
        print(f"RESULT  : Tc = {tc:.3f} s  (plateau, window {win:.0f} ms)   theory {theory:.3f}   ratio {ratio:.2f}")
    else:
        print(f"RESULT  : NO PLATEAU — resolution limited (Tc keeps rising with window)")
        print(f"          shortest-window Tc = {sweep[0][1]:.3f} s is a lower bound; need faster SRS to resolve")

...
case "$1" in
  ...
  save)
    ...
    # ---- rnti -> IMSI correlation ----
    # Produces ue_sessions.json (IMSI -> its rnti sessions this run). This is
    # what lets rb_to_ru.py label each record with its real user instead of
    # dumping everyone into "user 0". Requires: gNB detail log captured,
    # 'csi_run.sh start' having recorded a start time, and passwordless sudo
    # for journalctl set up on $AMF_HOST (see correlate_rnti_imsi.py --help).
    SESSIONS=""
    if [ -s "$GL" ]; then
      ...
      echo "WARNING: $GL was not captured this run — can't correlate rnti to IMSI. Falling back to single-user (u=0) output." >&2
    fi

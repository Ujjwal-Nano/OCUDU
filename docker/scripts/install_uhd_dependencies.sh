...
install_uhd_dependencies_fedora() {
    ...
    local -a run_pkgs=(
        kernel-tools iputils ncurses libusb1 python3-requests boost-atomic
        boost-chrono boost-container boost-date-time boost-filesystem boost-program-options
        boost-serialization boost-thread
    )
    local -a extra_pkgs=(

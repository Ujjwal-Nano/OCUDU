...
struct cg_configuration {
  ...
  /// (Type 1 CG). When absent, the grant must be activated via DCI (Type 2 CG).
  ///
  ...
  uint8_t nrof_harq_processes;
  /// Number of PUSCH repetitions per CG occasion and their RV sequence.
  repetitions_t rep;
  /// Periodicity of the CG resource.
  periodicity_t periodicity;
  /// Duration (in slots) of the configured-grant timer, used to detect CG PUSCH inactivity. Values {1,...,64}.
  /// \remark See TS 38.321, clause 5.8.2.
  ...
  bool operator==(const cg_configuration& rhs) const
  {
    return cs_rnti == rhs.cs_rnti && frequency_hopping == rhs.frequency_hopping && cg_dmrs_cfg == rhs.cg_dmrs_cfg &&
           ...
           nrof_harq_processes == rhs.nrof_harq_processes && rep == rhs.rep && periodicity == rhs.periodicity &&

...
void ocudu::declare_du_low_appconfig_schema(config::config_builder& root, du_low_appconfig& config)
{
  ...
  configure_cli11_with_logger_appconfig_schema(root, config.log_cfg);
  configure_cli11_with_tracer_appconfig_schema(root, config.trace_cfg);
  configure_cli11_with_worker_manager_appconfig_schema(root, config.expert_execution_cfg);

  app_services::configure_cli11_with_executor_metrics_appconfig_schema(root, config.metrics_cfg.executors_metrics_cfg);
  app_services::configure_cli11_with_app_resource_usage_config_schema(root, config.metrics_cfg.rusage_config);
  app_services::configure_cli11_with_metrics_appconfig_schema(root, config.metrics_cfg.metrics_service_cfg);

  configure_cli11_with_remote_control_appconfig_schema(root, config.remote_control_config);

#ifdef DPDK_FOUND
  config.hal_config.emplace();
  configure_cli11_with_hal_appconfig_schema(root, *config.hal_config);
#endif
}

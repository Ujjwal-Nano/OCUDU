...
static expected<std::chrono::system_clock::time_point, std::string> parse_epoch_time(const nlohmann::json& json)
{
  ...
  if (epoch_field->is_number()) {
    ...
    epoch_ms = epoch_ms_result->get<uint64_t>();
...
{
  std::variant<ecef_coordinates_t, orbital_coordinates_t> epmeheris_info;
  ecef_coordinates_t                                      ecef_coords;
  orbital_coordinates_t                                   orbital_params;
  auto                                                    ephemeris_info_result = json.find("ephemeris_info");
  if (ephemeris_info_result == json.end()) {
    return make_unexpected("'ephemeris_info' object is missing and it is mandatory");
  }
  if (ephemeris_info_result->contains("ecef")) {
    auto result = validate_json<ecef_coordinates_t>(*ephemeris_info_result, "ecef");
    if (!result) {
      return make_unexpected(result.error());
    }
    ecef_coords    = *result;
    epmeheris_info = ecef_coords;
    return epmeheris_info;
  }
  if (ephemeris_info_result->contains("orbital")) {
    auto result = validate_json<orbital_coordinates_t>(*ephemeris_info_result, "orbital");
    if (!result) {
      return make_unexpected(result.error());
    }
    orbital_params = *result;
    epmeheris_info = orbital_params;
    return epmeheris_info;
  };
...
error_type<std::string> ntn_config_update_remote_command::execute(const nlohmann::json& json)
{
  ...
  if (json.contains("common_ntn_config")) {
    auto common_json = json.find("common_ntn_config");
    if (common_json == json.end()) {
      return make_unexpected("'common_ntn_config' field is malformed");
    }
    ...
    auto             common_result = parse_cell_ntn_config(*common_json, dummy_plmn, dummy_nci, nullptr);
    if (!common_result) {

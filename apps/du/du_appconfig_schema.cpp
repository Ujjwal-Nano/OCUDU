...
void ocudu::declare_du_appconfig_schema(config::config_builder& root, du_appconfig& du_cfg)
{
  ...
  declare_logger_appconfig_schema(root, du_cfg.log_cfg);
  declare_tracer_appconfig_schema(root, du_cfg.trace_cfg);
  app_services::declare_buffer_pool_appconfig_schema(root, du_cfg.buffer_pool_config);
  declare_worker_manager_appconfig_schema(root, du_cfg.expert_execution_cfg);
  ...
  root.group("f1ap", "F1AP interface configuration", [&](config::config_builder& f1ap_b) {
    ...
    declare_sctp_socket_options(f1ap_b, du_cfg.f1ap_cfg.sctp);
  });
  ...
  root.group("f1u", "F1-U interface configuration", [&](config::config_builder& f1u_b) {
    ...
    declare_f1u_sockets_options(f1u_b, du_cfg.f1u_cfg.f1u_sockets);
  });
  ...
  app_services::declare_app_resource_usage_config_schema(root, du_cfg.metrics_cfg.rusage_config);
  app_services::declare_metrics_appconfig_schema(root, du_cfg.metrics_cfg.metrics_service_cfg);

#ifdef DPDK_FOUND
  du_cfg.hal_config.emplace();
  declare_hal_appconfig_schema(root, *du_cfg.hal_config);
#endif

  declare_remote_control_appconfig_schema(root, du_cfg.remote_control_config);
}

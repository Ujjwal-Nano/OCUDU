...
void ocudu::declare_cu_cp_appconfig_schema(config::config_builder& root, cu_cp_appconfig& cu_cp_cfg)
{
  ...
  declare_logger_appconfig_schema(root, cu_cp_cfg.log_cfg);
  declare_tracer_appconfig_schema(root, cu_cp_cfg.trace_cfg);
  app_services::declare_buffer_pool_appconfig_schema(root, cu_cp_cfg.buffer_pool_config);
  declare_worker_manager_appconfig_schema(root, cu_cp_cfg.expert_execution_cfg);
  declare_remote_control_appconfig_schema(root, cu_cp_cfg.remote_control_config);
  app_services::declare_app_resource_usage_config_schema(root, cu_cp_cfg.metrics_cfg.rusage_config);
  app_services::declare_metrics_appconfig_schema(root, cu_cp_cfg.metrics_cfg.metrics_service_cfg);
  ...
  root.group("cu_cp", "CU-CP parameters", [&](config::config_builder& cu_cp_b) {
    cu_cp_b.group("e1ap", "E1AP parameters", [&](config::config_builder& e1ap_b) {
      ...
      declare_sctp_socket_options(e1ap_b, cu_cp_cfg.e1ap_cfg.sctp);
    });
    cu_cp_b.group("f1ap", "F1AP parameters", [&](config::config_builder& f1ap_b) {
      ...
      declare_sctp_socket_options(f1ap_b, cu_cp_cfg.f1ap_cfg.sctp);
    });
  });
}

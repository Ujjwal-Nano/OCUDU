...
void ocudu::app_services::configure_cli11_with_buffer_pool_appconfig_schema(config::config_builder& b,
                                                                            buffer_pool_appconfig&  config)
{
  ...
  app_helpers::configure_cli11_with_metrics_appconfig_schema(b, config.metrics_config.common_metrics_cfg);
}

void ocudu::app_services::configure_cli11_with_buffer_pool_appconfig_schema(CLI::App& app, buffer_pool_appconfig& config)
{
  config::schema_node discard;
  discard.body = config::group_node{};
  config::config_builder b(app, discard);
  configure_cli11_with_buffer_pool_appconfig_schema(b, config);
}
